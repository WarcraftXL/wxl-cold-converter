...
namespace wxl::modern::assets::wmo
{
    ...
    namespace
    {
        Options g_opts; // set once via Configure() before any Translate call
        ...
        // Source shader id -> the client's exterior effect table index. The client indexes the effect
        // table directly by this field, unclamped: an id past the table reads a garbage handle and faults
        // the group draw, and the exterior table's slot 6 is null (only the interior table has a two-layer
        // composite effect). A single material record has no interior/exterior context of its own (the
        // same material can be drawn by both), so every id below targets what is safe for an exterior
        // batch, the one choice that is never wrong regardless of which group ends up drawing it. 0..5 are
        // already native; the rest picks the nearest trait the exterior table can express: env reflection
        // (3/5/7/11/12/17) keeps texture_1/texture_2 as-is (texture_2 already holds a usable second texture
        // for this family, no slot promotion needed), everything else falls to the plain diffuse/opaque
        // base layer already sitting in texture_1. Index 23 is additionally special-cased by the caller
        // (its base texture is promoted into texture_1 first).
    ...
    void Configure(const Options& options) { g_opts = options; }
    ...
    bool TranslateWmoRoot(std::span<const uint8_t> in, const ResolveCtx& rc, std::vector<uint8_t>& out)
    {
        ...
        if (momtC.data)
        {
            ...
            for (uint32_t i = 0; i < matCount; ++i)
            {
                ...
                // Every source shader goes through the same nearest-match table, not just ids past 6:
                // forcing all shaders to 0 unconditionally would flatten natively-renderable env/specular/
                // metal materials (1/2/3/5) to plain diffuse too.
            ...
            // Info (not Printf): per-file chatter would drown the batch progress bar (see Runner.cpp);
            // wxl-core's own copy of this file logs unconditionally since it has no bar to protect.
            wxl::core::log::Info("wmo-mat: mats=%u collapsed=%u mat0 shader=%u tex1=%s tex2=%s",
        ...
        if (g_opts.showDoodads && modiC.data && moddC.data && moddC.size >= kModdStride)
        {
            ...
            rebuiltDoodads = true;
            wxl::core::log::Info("wmo-doodad: MODI %u (resolved %u) -> MODN names, %u defs (blob %u B)",
        ...
        // A disabled toggle only forces a rebuild when the source actually carries the chunk it hides;
        // otherwise there is nothing to strip and the early bail-out below still applies.
        iff::Chunk probe{};
        const bool needsToggleStrip =
            (!g_opts.showDoodads    && (moddC.data || modiC.data)) ||
            (!g_opts.includePortals && (reader.Find(kMOPV, probe) || reader.Find(kMOPT, probe) ||
                                         reader.Find(kMOPR, probe) || reader.Find(kMOVV, probe) ||
                                         reader.Find(kMOVB, probe))) ||
            (!g_opts.includeLights  && reader.Find(kMOLT, probe));
        ...
        if (!strippedUnknown && !modern && fdidResolved == 0 && fdidFallback == 0 &&
            !createdMotx && !runTimeDirty && !shaderRemapped && !needsToggleStrip)
            return false;
        ...
        // User toggles that hide whole chunk families regardless of what the source shipped: force an empty
        // chunk instead of falling through to the source-passthrough/rebuild paths below.
        auto forcedEmpty = [&](uint32_t magic) -> bool
        {
            if (!g_opts.showDoodads && (magic == kMODN || magic == kMODD || magic == kMODS)) return true;
            if (!g_opts.includePortals && (magic == kMOPV || magic == kMOPT || magic == kMOPR ||
                                            magic == kMOVV || magic == kMOVB)) return true;
            if (!g_opts.includeLights && magic == kMOLT) return true;
            return false;
        };

        for (uint32_t magic : kCanonicalRoot)
        {
            if (forcedEmpty(magic)) { iff::Emit(out, magic, nullptr, 0); continue; }
    ...
    bool TranslateWmoGroup(std::span<const uint8_t> in, std::vector<uint8_t>& out)
    {
        ...
        if (!g_opts.includeCollision) { hasMOBN = false; hasMOBR = false; }
        if (!g_opts.includeLiquid)    hasMLIQ = false;
        ...
        if (hasMOCV)
        {
            ...
            if (g_opts.neutralizeVertexColors)
            {
