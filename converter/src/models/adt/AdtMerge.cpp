...
namespace wxl::modern::adt
{
    namespace
    {
        Options g_opts; // set once via Configure() before any MergeSplitAdt() call
        ...
        void buildMcnk(Out& o, int id,  const uint8_t* rb, uint32_t rOff, uint32_t rLen, const uint8_t* tb, uint32_t tOff, uint32_t tLen, const uint8_t* ob, uint32_t oOff, uint32_t oLen, uint32_t mcin[256][2], std::vector<RawExtraChunk>& extras)
        {
            ...
            if (tb && sub(tb, tOff, tLen, MCLY, d, s))
            {
                ...
                for (int i = 0; i < keep; ++i)
                {
                    ...
                    const uint32_t ground = g_opts.preserveGroundEffectId ? rd32(L+0x0C) : 0;
                    uint32_t newOfs = 0;
                ...
                if (layers > 4 && g_opts.packExtraTextureLayers)
                {
                    ...
                    for (int i = 4; i < last; ++i)
                    {
                        ...
                        el.ground    = g_opts.preserveGroundEffectId ? rd32(L+0x0C) : 0;
            ...
            uint64_t hiHoles = (g_opts.highResHoles && (flags & 0x10000)) ? rd64(rh + 0x14) : 0;
            ...
            // +0x34 areaId: kept from the copied root header (zeroed below when disabled)
            if (!g_opts.preserveAreaId) o.patch32(hdrPos + 0x34, 0);
                       ...
                       const ResolveCtx& rc)
    {
        ...
        wxl::core::log::Info("adt: merge %.*s hasTex=%d hasObj=%d rootMcnk=%d tex=%d obj=%d",
        ...
        else if (hasTex && find(tb, tl, MDID, d, s) && s >= 4)
        {
            ...
            nTexture = int(count);
            wxl::core::log::Info("adt: %.*s MDID->MTEX %u textures (resolved=%u fallback=%u)",
        ...
        if (legacyNames)
        {
            ...
            if (hasObj && g_opts.showDoodads && find(ob, ol, MDDF, d, s)) {
            ...
            // MODF: clear flags 0x4/0x8. The per-instance scale (0x3E, 1024 = x1.0) is PRESERVED by default;
            // the Client ignores it as padding, and the dynamic_scaling runtime reads it to scale the WMO.
            if (hasObj && g_opts.showWmoObjects && find(ob, ol, MODF, d, s)) {
                ofsMODF = o.tell(); o.u32(MODF); o.u32(s); uint32_t b2=o.tell(); o.raw(ob+d, s);
                for (uint32_t e=0;e+64<=s;e+=64) {
                    o.patch16(b2+e+0x38, rd16(&o.d[b2+e+0x38]) & ~0xC);
                    if (!g_opts.preserveWmoScale) o.patch16(b2+e+0x3E, 1024);
                }
        ...
        else
        {
            ...
            if (hasObj && g_opts.showDoodads && find(ob, ol, MDDF, md, ms)) dood = rebuildFdidPlacements(ob+md, ms, 36, 0x22, 0x0007, "missing.m2", rc);
            if (hasObj && g_opts.showWmoObjects && find(ob, ol, MODF, od, oz))
            {
                // Per-instance scale at 0x3E is PRESERVED by default (the dynamic_scaling runtime applies it).
                wmo = rebuildFdidPlacements(ob+od, oz, 64, 0x38, uint16_t(~0xC), "missing.wmo", rc);
                if (!g_opts.preserveWmoScale)
                    for (uint32_t e = 0; e + 64 <= wmo.entries.size(); e += 64)
                        { wmo.entries[e+0x3E] = uint8_t(1024 & 0xFF); wmo.entries[e+0x3F] = uint8_t(1024 >> 8); }
            }
            ...
            wxl::core::log::Info("adt: %.*s FDID placements doodads=%u(miss %u) wmo=%u(miss %u)",
                int(name.size()), name.data(), dood.resolved, dood.missing, wmo.resolved, wmo.missing);
        }

        if (g_opts.fixWater && find(rb, rl, MH2O, d, s)) { std::vector<uint8_t> w = fixMh2o(rb+d, s); ofsMH2O = o.chunk(MH2O, w.data(), uint32_t(w.size())); }
        ...
        // MTXF (texture flags): from tex0 MTXF, else derived from MTXP, else one zero per texture.
        {
            ...
            // tile ships both chunks; checking MTXF first would drop the height params and scale
        ...
        // height data (a height texture, or a non-default scale).
        {
            ...
            if (g_opts.heightBlendTable && !heights.empty())
            {
                std::vector<uint8_t> payload;
                SerializeTexHeights(heights, payload);
                o.chunk(kATHB, payload.data(), uint32_t(payload.size()));
                wxl::core::log::Info("adt: %.*s ATHB %u height-blend textures",
        ...
        if (!extras.empty())
        {
            ...
            if (!table.empty())
            {
                ...
                wxl::core::log::Info("adt: %.*s ATL2 %u chunks, %u extra layers",
