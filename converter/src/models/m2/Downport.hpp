...
{
    // User-facing toggles for the downport. Every field defaults to true, so a default-constructed
    // Options enables every optional transform.
    // Cameras/particles/textures are NOT toggleable here: their compaction is a stride/index fix the client
    // needs whenever the source carries that data, not a cosmetic choice (see AnimationFix/RibbonCompact).
    struct Options
    {
        bool animationFix    = true; // remap sequence ids the client can't resolve (>505) to a safe fallback
        bool ribbonCompact   = true; // downport ribbon emitter records (index-bounds clamp)
    };

    /** @brief Sets the toggles used by every ProcessInPlace() call. Call once before converting a batch. */
