...
namespace wxl::modern::assets::m2::md21
{
    ...
    namespace
    {
        ...
        // The shadow-swing case this handles is a static prop rig (a lamp/torch's body on a static root
        // plus its glow on a billboard bone, e.g. boneCombos=[0,1,0,1,...]): at most 2 distinct bones. A
        // real skeleton (creature/character) references dozens to hundreds, so this margin comfortably
        // separates the two without needing to walk animation track data.
    ...
    bool Dechunk(std::span<const uint8_t> in, TxidResolver resolve, std::vector<uint8_t>& out)
    {
        ...
        // Info (not Printf): one line per model would drown the batch progress bar; muted during a
        // folder run (see Runner.cpp), still shown for a single-file conversion. wxl-core's own copy of
        // this file logs unconditionally since it has no bar to protect -- deliberate divergence here.
        wxl::core::log::Info("modern-m2 md21: md20=%u textures=%u hardcoded=%u inlined=%u txid=%zu",
