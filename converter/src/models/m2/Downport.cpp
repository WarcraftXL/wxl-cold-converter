...
namespace wxl::modern::assets::m2::downport
{
    ...
    namespace { Options g_opts; } // set once via Configure() before any ProcessInPlace() call
    ...
    bool ProcessInPlace(void* work, uint32_t origSize, uint32_t workSize)
    {
        ...
        if (g_opts.ribbonCompact) ribbons::Compact(md, origSize);
        if (g_opts.animationFix)  animations::Fix(md);
